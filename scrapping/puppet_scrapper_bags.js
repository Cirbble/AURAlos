...
class AldoBagsScraper {
    ...
    async scrapeProducts() {
        console.log('Navigating to Aldo bags collection...');
        ...
        if (!productSelector) {
            ...
            await this.page.screenshot({ path: path.join(__dirname, 'debug_bags_screenshot.png'), fullPage: true });
            
            // Save HTML for debugging
            const html = await this.page.content();
            fs.writeFileSync(path.join(__dirname, 'debug_bags.html'), html);
            console.log('Saved debug HTML to debug_bags.html');
            return;
        }
    ...
    async saveProgress() {
        const progressFile = path.join(__dirname, 'aldo_bags_progress.json');
    ...
    async saveResults() {
        const outputFile = path.join(__dirname, 'aldo_bags_products.json');
    ...
    async run() {
        try {
            console.log('🚀 Starting Aldo bags scraper...');
            await this.init();
            ...
            try {
                if (this.page) {
                    await this.page.screenshot({ path: path.join(__dirname, 'error_bags_screenshot.png'), fullPage: true });
...
// Run the scraper
const scraper = new AldoBagsScraper();
scraper.run().then(() => {
