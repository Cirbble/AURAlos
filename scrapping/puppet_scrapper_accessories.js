...
class AldoAccessoriesScraper {
    constructor() {
        this.baseUrl = 'https://www.aldoshoes.com/en-ca/collections/womens-accessories';
        this.products = [];
        this.imageDir = path.join(__dirname, 'images_accessories');
    }
    ...
    async scrapeProducts() {
        console.log('Navigating to Aldo accessories collection...');
        ...
        if (!productSelector) {
            ...
            await this.page.screenshot({ path: path.join(__dirname, 'debug_accessories_screenshot.png'), fullPage: true });
            
            // Save HTML for debugging
            const html = await this.page.content();
            fs.writeFileSync(path.join(__dirname, 'debug_accessories.html'), html);
            console.log('Saved debug HTML to debug_accessories.html');
            return;
        }
    ...
    async saveProgress() {
        const progressFile = path.join(__dirname, 'aldo_accessories_progress.json');
    ...
    async saveResults() {
        const outputFile = path.join(__dirname, 'aldo_accessories_products.json');
    ...
    async run() {
        try {
            console.log('🚀 Starting Aldo accessories scraper...');
            await this.init();
            ...
            try {
                if (this.page) {
                    await this.page.screenshot({ path: path.join(__dirname, 'error_accessories_screenshot.png'), fullPage: true });
...
// Run the scraper
const scraper = new AldoAccessoriesScraper();
scraper.run().then(() => {
